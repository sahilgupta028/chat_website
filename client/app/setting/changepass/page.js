...
const page = () => {
  ...
  const handleQues = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/profile/change`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Password changed failed:', data.error || 'Unknown error');
        setError(data.error || 'Unknown error');
      }
    } catch (error) {
      console.error('Error during Password changed:', error.message || 'Unknown error');
        ...
        <div className="max-w-md w-full space-y-8 p-6 bg-gray-900  border border-gray-200 rounded-lg">
          ...
          <form className="mt-8 space-y-6">
            <div className="rounded-md shadow-sm -space-y-px">
              <div>
                <label htmlFor="password" className="block text-lg font-semibold text-white">Old Password</label>
                <input id="password" name="password" type="text" required value={password} onChange={(e)=>setPassword(e.target.value)} className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500  rounded-t-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm text-black" placeholder="Old Password..." />
              </div>
            </div>
            <div className="rounded-md shadow-sm -space-y-px">
              <div>
                <label htmlFor="newPassword" className="block text-lg font-semibold text-white">New Password</label>
                <input id="newPassword" name="newPassword" type="text" required value={newPassword} onChange={(e)=>setNewPassword(e.target.value)} className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500  rounded-t-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm text-black" placeholder="New Password..." />
              </div>
            </div>
            <div>
